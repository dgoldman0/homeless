...
with
    ...
    beta^l = -U_coord(X) * E(X) * W(l)^p_beta * S(l-X)
    U_coord(X) = U_proper(X) / B_v1(X)
...
It computes:
  ...
  - packet worldtube norm: -alpha^2 + gamma_ll (U_coord + beta^l)^2
...
Recommended local command:
    ...
    python adm_3p1_viability_v3_baware.py --atlas --outdir adm_outputs --nl 81 --nth 33 --nph 8

A stronger single-slice run:

    python adm_3p1_viability_v3_baware.py --x0 0.25 --cycle-phase hold_mid --V 5 --lambda-factor 5.75 \
        --nl 121 --nth 49 --nph 12 --outdir adm_slice_V5

A high-stress single-slice run:

    python adm_3p1_viability_v3_baware.py --x0 0.25 --cycle-phase hold_mid --V 10 --lambda-factor 6 \
...
def evaluate_slice(
    ...
    U_coord = fields["packet_U_coord"]
    packet_norm = -alpha*alpha + gamma[..., 0, 0]*(U_coord + beta_up[..., 0])**2
    ...
    summary = {
        ...
        "packet": {
            "U_proper": float(fields["packet_U_proper"]),
            "U_coord": float(fields["packet_U_coord"]),
...
def compact_status(summary: Dict):
    ...
    return {
        ...
        "U_proper": summary["packet"]["U_proper"],
        "U_coord": summary["packet"]["U_coord"],
        "q": summary["packet"]["q"],
        "E": summary["packet"]["E"],
