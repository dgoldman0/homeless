# Catch/release transition-layer refinement memo

## Result

The composite design now has a concrete transition-layer refinement target. The stressed case uses $V=10$, $\lambda=6$, exact v1 throat controls, R-open service posture, B-aware packet choreography, paired capacity/lapse support, and ADM source-demand proxies.

The selected candidate is `earlycatch_medium_p4`.

```math
x_{catch}=0.05,\quad x_\beta=0.70,\quad x_q=1.25
```

```math
w_{catch}=0.25,\quad w_\beta=0.28,\quad w_q=0.30,\quad p_\beta=4.0
```

The selected candidate clears packet, support-edge, and release-edge screens in the confirmation grid. Its source-demand cost is `0.939` of the baseline reference.
...
The refinement pass shows the useful direction:

```math
\text{early catch} + \text{medium transition widths} + \text{strong edge shaping}
```

The selected candidate keeps `p_beta=4.0` and moves catch earlier. The medium-width profiles reduce transition source-current demand while preserving edge clearance.
...
The next composite design should use this service-layer structure:
...
with medium-width transition profiles and R held open through packet service.
